# Contributing to the GS1 WebOntology Dictionary
...
foundation for linked data, and collaboration is key to
its success.
...
2.  **Proposing New Terms:** If a required attribute is missing from the dictionary, propose its addition.
3.  **Defining Equivalencies:** Help identify and map semantic equivalencies between a dictionary and external standards (e.g., ISO, IEC, specific industry vocabularies).
...
7.  **Open a Pull Request (PR)** against the `stage` branch of this repository.
...
-   **Automatic Review Assignment:** The project uses a `.github/CODEOWNERS` file to automatically assign the project owner (@Ben-GS1) to review all pull requests.
...
*   **Sector-/Domain related Definitions (`src/sectors/`):** The semantic source of truth. sector-/ domain-specific definitions.
*   **Ontology Definitions (`src/shared/`):** The semantic source of truth. Contains all the ontologies shared over multiple Sectors and Domains, like the Discovery Service related terms..
*   **Scrips Files (`src/scripts/`):** Scripts to generate the manifest - 
