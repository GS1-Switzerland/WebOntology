/**
 * GS1 Sector code list. This is the one piece of near-static configuration
 * the brief calls out explicitly — everything else (domains, artifacts,
 * terms) is loaded at runtime from the JSON-LD manifest published on
 * GitHub Pages. Labels here are the canonical English source strings and
 * are addressed via i18n keys (sector.<codeValue>) so they can be
 * translated without touching this file — see src/i18n/locales/en/sectors.json.
 */
...
export const GS1_SECTORS: Gs1Sector[] = [
...
export const sectorByCode = (code: string): Gs1Sector | undefined =>
  GS1_SECTORS.find((s) => s.codeValue.toLowerCase() === code.toLowerCase());
